...
class Discretizer(object):
    ...
    def __init__(self, data: np.ndarray, numerical_features: List[int], feature_names: List[str],
        ...
        for feature, linspace in zip(self.to_discretize, bins):
            # get nb of borders (nb of bins - 1) and the feature name
            n_bins = linspace.shape[0]
            name = feature_names[feature]

            # create names for bins of discretized features
            self.feature_intervals[feature] = ['%s <= %.2f' % (name, linspace[0])]
            for i in range(n_bins - 1):
                self.feature_intervals[feature].append('%.2f < %s <= %.2f' % (linspace[i], name, linspace[i + 1]))
            self.feature_intervals[feature].append('%s > %.2f' % (name, linspace[n_bins - 1]))
            self.lambdas[feature] = partial(self.get_percentiles, linspace=linspace)

    @staticmethod
    def get_percentiles(x: np.ndarray, linspace: np.ndarray) -> np.ndarray:
        if len(linspace.shape) != 1:
            raise ValueError("Expected 1D quantiles array!")
        if not issorted(linspace):
            raise ValueError("Linspace array should be sorted!")
        return np.searchsorted(linspace, x)
